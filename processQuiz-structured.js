...
// 遍歷所有題目，以更結構化的 HTML 格式處理 solution
  ...
  if (obj.questions && Array.isArray(obj.questions)) {
    obj.questions.forEach(question => {
      ...
      if (question.solution && typeof question.solution === 'string') {
        // 使用結構化 HTML，而非簡單的 <br> 標籤
        let formattedSolution = `<div class="solution-content"><p><strong>(${correctAnswer}) 正確。</strong> `;
        ...
        if (parts.length > 1) {
          ...
          formattedSolution += `${correctExplanation}</p><div class="options-explanation">`;
          ...
          for (let i = 0; i < parts.length; i++) {
            if (['A', 'B', 'C', 'D'].includes(parts[i]) && i < parts.length - 1) {
              ...
              formattedSolution += `<p><strong>(${optionKey})</strong> ${explanation}</p>`;
            }
          }
          
          formattedSolution += '</div></div>';
        } else {
          // 沒找到選項分割點，直接使用原始 solution 但包裝在 <p> 標籤中
          formattedSolution += `${solutionText}</p></div>`;
        }
...
console.log('已成功更新 L21.json 檔案，使用了結構化 HTML 格式'); 